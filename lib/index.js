  ...
  return new Promise((resolve) => {
    ...
    if (!notest && !(config && config.includes('notest'))) {
      if (config && config.includes('spec')) {
        files.push(`${name}.spec.${isJsxFile ? jsxExt : ext}`);
      } else {
        files.push(`${name}.test.${isJsxFile ? jsxExt : ext}`);
      }
    }

    // Add Storie for storbook
    if (stories || (config && config.includes('stories'))) {
      files.push(`${name}.stories.${isJsxFile ? jsxExt : ext}`);
    }
      ...
      .then(() => {
        ...
        for (let i = 0; i < files.length; i += 1) {
          ...
          if (file === indexFile) {
            ...
            file.indexOf(`.spec.${isJsxFile ? jsxExt : ext}`) > -1
            || file.indexOf(`.test.${isJsxFile ? jsxExt : ext}`) > -1
          ) {
            data = createTest(
              name,
          ...
          if (
            (file.indexOf(`.stories.${isJsxFile ? jsxExt : ext}`)
              || file.indexOf(`.stories.${isJsxFile ? jsxExt : ext}`)) > -1
          ) {
